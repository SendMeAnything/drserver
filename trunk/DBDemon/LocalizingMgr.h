...
private:
	...
	int m_iRookieAbleLv;//초보자 서버 가능 레벨
	int m_iDoubtUserTableLimitDay;//다우트 유저 테이블 지우는 최소 기간
private:
...
*/
	void SetRookieLv(const int iValue) {m_iRookieAbleLv = iValue;}
	int GetRookieLv()const{return m_iRookieAbleLv;}

	void SetDoubtUserTableLimitDay(const int iValue) {m_iDoubtUserTableLimitDay = iValue;} 
	int GetDoubtUserTableLimitDay()const{return m_iDoubtUserTableLimitDay;}
};
