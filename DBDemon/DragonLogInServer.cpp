...
//-----------------------------------------------------------------------------
//acer7
int CheckMyCode( int code, char *mystring, char *mystring1 )
{
	static int count = strlen( MY_STRING );
	static int count1 = strlen( MY_STRING1 );
	if(!LocalMgr.IsAbleMyCode(code)) {return false;}//021007 lsw
	if( strncmp( MY_STRING, mystring, count ) != 0 ) return false;
	if( strncmp( MY_STRING1, mystring1, count1 ) != 0 ) return false;
	return true;
}
...
int CheckIDAutherizing(t_packet &packet, const int cn)
{
	...
	char *szMyString1	= packet.u.ClientAccessLogin.mystring1;
	...
	if( !::CheckMyCode( code, szMyString, szMyString1 ) ) 
	{
		packet.h.header.type = CMD_INVALID_VERSION;
		packet.h.header.size = 0;
		::QueuePacket(connections, cn, &packet, 1);
